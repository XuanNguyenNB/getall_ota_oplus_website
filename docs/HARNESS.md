...
managed by the Rust Harness CLI through the stable repo-local entrypoint
`scripts/harness`. Agents and humans should use that entrypoint for Harness
work. The database is local to each project instance and `.gitignore`d. The
schema is version-controlled under `scripts/schema/`.
...
6. Before finishing, ask whether product truth, validation expectations,
   ...
   changed.
7. Record a trace with `scripts/harness trace`.
