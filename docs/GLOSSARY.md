...
Anonymous visibility plus guarded active actions: Cloudflare Tunnel ingress,
server-validated Turnstile, quota/cooldown enforcement and authenticated
operator routes.

## Resolver Proof Gate

The requirement that a bounded real component-link flow be validated before
the resolver endpoint can be enabled for users.
