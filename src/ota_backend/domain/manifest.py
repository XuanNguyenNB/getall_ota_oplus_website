...
AUTHORITATIVE_MANIFEST_TARGETS: dict[str, ManifestTarget] = {
    code: ManifestTarget(
        code=code,
        nv_id=nv_id,
        # The tuple literals below carry int values; mypy narrows them to the
        # ServerRegion Literal via the typed sequence annotation.
        server_region=server_region,
        server_region_label=label,
        source="Universal OTA DownloadeR REGIONS/SERVERS map; maintainer-approved live source",
    )
    for code, nv_id, server_region, label in cast(
        "tuple[tuple[str, str, ServerRegion, str], ...]",
        (
            ("00", "00000000", 3, "EX"),
            ("A4", "10100100", 3, "APC"),
            ("A5", "10100101", 3, "OCA"),
...
def manifest_blockers() -> list[str]:
    return [code for code in ACCEPTED_MANIFEST_CODES if code not in AUTHORITATIVE_MANIFEST_TARGETS]
